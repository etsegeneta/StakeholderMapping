...
.service('DateUtils', function ($filter, CalendarService) {
    ...
    return {
        ...
        getPeriodDate: function( period, prop ){
            if (!period || !prop || !period[prop]){
                return;
            }

            var d = period[prop]._day, m = period[prop]._month, y = period[prop]._year;
            d = d < 10 ? '0'+ d : d;
            m = m < 10 ? '0'+ m : m;
            return y + '-' + m + '-' + d;
        },
...
.service('CommonUtils', function($q, $translate, $filter, SessionStorageService, DateUtils, OptionSetService, CurrentSelection, FileService, DialogService, OrgUnitService){
    ...
    return {
            ...
            if( de.optionSetValue ){
                ...
                if( val ){
                    if( de.valueType === 'NUMBER' && dhis2.validation.isNumber(val) ){
                        ...
                        val = val === 'true' ? destination === 'USER' ? 'Yes' : true : '';
                    }
                    else if(de.valueType=== 'BOOLEAN'){
                        val = val === 'true' || val === true ? destination==='USER' ? 'Yes' : true : val === 'false' || val === false ? destination==='USER' ? 'No' : false : '';
                    }
                }
            }
        ...
        getOptionComboIdFromOptionNames: function(categoryCombo, options){
            ...
            if( categoryCombo && categoryCombo.categoryOptionCombos && categoryCombo.categoryOptionCombos.length > 0 && options && options.length > 0 ){
                for( var j=0; j< categoryCombo.categoryOptionCombos.length; j++){
                    var ops = categoryCombo.categoryOptionCombos[j].displayName.split(',');
                    var match = true;
                    if ( ops && ops.length && ops.length > 0 ){
                        for( var i=0; i<options.length; i++ ){
                            if( ops.indexOf(options[i].displayName) === -1 ){
                                match = false;
                                break;
                            }
                        }
        ...
        errorPostNotifier: function(response){
            if( response && response.status === 'ERROR'){
                var dialogOptions = {
                    headerText: response.status,
                    bodyText: response.response && response.response.importSummaries && response.response.importSummaries[0].description ? response.response.importSummaries[0].description : response.message
                };
                DialogService.showDialog({}, dialogOptions);
            }
        },
